...
import { MemoryStat } from '../interface/system-memory-stats';
...
export class DashboardComponent implements OnInit {
  ...
  public systemCPUPercent: SystemCPU;
  public systemMemory: MemoryStat;
  ...
  private getCPUPercent() {
    this.dashboardService.getCpuPercent().subscribe(
      (response: SystemCPU) => {
        this.systemCPUPercent = response;
      },
      (error: HttpErrorResponse) => {
        alert(error.message);
      }
  ...
  private getMemory() {
    this.dashboardService.getMemorySystem().subscribe((memoryStat: MemoryStat) => {
      this.systemMemory = memoryStat;
    });
  }
  ...
  private processTraces(traces: any) {
    ...
    this.traceList = traces.filter((trace) => {
      return !trace.request.uri.includes('actuator') && !trace.request.uri.includes('memory-status');
    });
